...
bit Time_Up1,Time_Up2,Time_Up3,Time_Up4;										//延时时间到标志位
...
{
	Key_Cheak=1;                       //开启按键检测---->>定时器2中完成检测和处理过程
	Key_Counter=0x0000;
	Key_Value=0;
...
/*----------------------可控硅调光控制输出程序------------------------*/	
	if(SCR1_control_flag||SCR2_control_flag)               //可控硅的调光控制
	{
...
/*----------------按键的读取，短按、长按、停止控制的判断，消抖-------------*/	
	if(Key_Cheak)				//开关使83编码器产生脉冲触发中断0--keycheak=1;
	{
		Key_Counter++;
//1		
		...
		{				
			...
			{
				if(A0)			//A0=P2^4;8-3编码器最低位
					Key_Value=Key_Value|0x01;			
				if(A1)			//A1=P2^5; 8-3编码器第二位
					Key_Value=Key_Value|0x02;		
				if(A2)			//A2=P2^6; 8-3编码器最高位
					Key_Value=Key_Value|0x04;	
					
				Key_Value=~Key_Value;          //74148的编码为反码
				Key_Value=Key_Value&0x07;	   //得到按键按下74148的端口数
			}
			else		//若已松开，则认为误操作
			{
				Key_Cheak=0;
...
//2		
		if(Key_Counter>KEY_SHAKE_TIME)               //按键前消抖之后开始判断按键是长按还是短按
		{
			...
			else if(Key_Counter==KEY_BOUNDARY_TIME) //长按
	...
	if(Sound_Delay1_Flag)
	{
		...
		if(Sound_Sensor_Counter==10000)					//1S
		{
			...
			if(Sound_Sensor_Counter1==Sound_Delay[0])
			{
				...
				Sound_Pack_Flag=1;				//计时时间到，置报文组装标志位
				Sound_Check_Flag=1;				//置声音传感器恢复检测标志位
				Sound_Pack2_Flag=1;				//延时后组装的报文标志位				
				Sound_Delay1_Flag=0;
			}
		}
	}
